...
train_encode_vec = '../../data/jubenwang_data/train_encode.vec' # 100MB
train_decode_vec = '../../data/jubenwang_data/train_decode.vec'
test_encode_vec = '../../data/jubenwang_data/test_encode.vec'
test_decode_vec = '../../data/jubenwang_data/test_decode.vec'

checkpoint_path = "chatbot_model/"
...
with tf.Session(config=config) as sess:
    ...
    model.saver.save(sess, checkpoint_path + 'ckpt', global_step=model.global_step)
    ...
    while True:
        ...
        if total_step % 500 == 0:
            ...
            print("loss="+str(loss), file=f_out)
            # 如果模型没有得到提升，减小learning rate
            if len(previous_losses) > 2 and loss > max(previous_losses[-3:]):
                sess.run(model.learning_rate_decay_op)
            previous_losses.append(loss)
            # 保存模型
            if total_step % 30000 == 0:
                model.saver.save(sess, checkpoint_path + 'ckpt', global_step=model.global_step)
            loss = 0.0
            # 使用测试数据评估模型
